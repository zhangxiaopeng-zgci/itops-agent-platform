...
## Phase 6a - Kite Kubernetes Console Integration

Goal:

```text
Use a mature lightweight Kubernetes dashboard instead of rebuilding Kubernetes resource management inside this platform.
```

Architecture decision:

```text
Kite runs as an independent Docker container.
AIOps Agent keeps Kubernetes cluster registration, topology correlation, Hermes diagnosis, approval, task execution, audit, and evolution governance.
Kite owns Kubernetes visual operations: resource browsing, logs, terminals, Helm, multi-cluster dashboard, and Prometheus-oriented views.
Rancher remains too heavy for the current single-host and operator-workbench direction.
```

Implemented first slice:

```text
docker-compose.hermes.yml adds a kite service using ghcr.io/kite-org/kite:latest.
Kite persists its sqlite data under /opt/itops-agent-platform/kite-data with DB_DSN=/data/db.sqlite.
Kite requires KITE_ENCRYPT_KEY from the deployment .env so container rebuilds keep a stable production-safe encryption key.
Kite is exposed on host port 3002 and attached to the itops-runtime network.
Backend readiness uses KITE_URL=http://kite:8080, KITE_PUBLIC_URL=http://10.1.132.58:3002, and a read-only KITE_DATA_DIR=/app/kite-data mount.
The frontend adds a Kubernetes Console page that opens Kite and attempts an embedded preview.
Navigation adds Kubernetes Console under the operator workspace.
Assets Center and Kubernetes Cluster Management link to the Kite console.
Production readiness now checks Kite reachability and Kite sqlite persistence.
```

Remaining:

```text
Decide authentication boundary: separate Kite login first, later evaluate SSO/proxy integration.
Decide whether Kite should manage kubeconfigs directly or consume a curated kubeconfig generated by the AIOps platform.
Add backup/restore drill coverage for kite-data beyond the current read-only persistence evidence.
Add Hermes output links that deep-link operators to the Kubernetes Console when a diagnosis references Kubernetes resources.
```

Acceptance:

```text
Operators can use Kite for Kubernetes resource management without the AIOps platform rebuilding a Kubernetes dashboard.
```
